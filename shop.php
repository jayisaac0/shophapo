							...
							<div class="caption">
								<div class="row">
									<div class="col-sm-12 col-md-12">
										...
										<div class="caption ">
										<h3 style = "text-align:center;font-family:Budmo Jiggler;"><b><a href = " offeredservices.php?id=<?php echo $user->user_id; ?> ">View services</a></b></h3>
										</div>
									</div>
								</div>
							</div>
...
<nav class="navbar navbar-default">
 <div class="container-fluid">
	...
	<?php
	    $stmt = $auth_user->runQuery("SELECT * FROM products WHERE user_id = $id GROUP BY Productcategory DESC");
	    $stmt->execute(array());
	    $products=$stmt->fetchAll(PDO::FETCH_OBJ);
	    foreach ($products as $product) {
	        ?>						
				<ul class="navbar-nav" style="padding:2px;margin:0px;">
					<li class="w3-animate-zoom w3-btn w3-light-grey w3-padding-4" style="width:100%;">
					<a href="category.php?id=<?php echo $user->user_id; ?>&amp;category=<?php echo stripcslashes(base64_encode($product->Productcategory)); ?>"  style="text-decoration:none;"><?php echo $product->Productcategory; ?></a>
					</li>
				</ul>
	...
	<?php
	    $stmt = $auth_user->runQuery("SELECT * FROM products WHERE user_id = $id ORDER BY timeposted DESC");
	    $stmt->execute(array());
	    $products=$stmt->fetchAll(PDO::FETCH_OBJ);
	    foreach ($products as $product) {
	        ?>						
	        	<div class="col-sm-12 col-md-3 myBorder" id = "contentsize">
					...
					<a href = "product.php?id=<?php echo $product->productid; ?> "><img src="<?php  echo $product->product_image; ?>" alt="joshua" style = "width:auto; height:200px;" class = "myHover" id = "productsContainerRadius" /></a>
						<div class="caption col-md-12 myBorder" id = "productAlignment" style = "height:auto;">
					        ...
					        <price class = "productsPriceColor">Price: $<?php echo number_format($product->productprice, 2); ?>   <b style="color:red;"> + VAT</b></price> <br />
